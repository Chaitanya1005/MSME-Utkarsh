...
// The voice pipeline is usable by BM, RM, and ZM (each scoped to their
// own branch/region/zone) — see the Full-Hierarchy Expansion plan, Phase
// 2. GM/CO never submits voice updates (read-only role). This is the
// single place that maps a role to its scope, replacing what used to be
// three separate hardcoded `user.role !== 'BM'` checks.
function resolveVoiceScope(user: AuthTokenPayload): VoiceScope {
  if (user.role === 'BM') {
    if (!user.branchId) throw new AuthorizationError('You have no branch assignment');
    return { branchId: user.branchId };
  }
  if (user.role === 'RM') {
    if (!user.regionId) throw new AuthorizationError('You have no region assignment');
    return { regionId: user.regionId };
  }
  if (user.role === 'ZM') {
    if (!user.zoneId) throw new AuthorizationError('You have no zone assignment');
    return { zoneId: user.zoneId };
  }
  throw new AuthorizationError('This role cannot submit voice updates');
}

// Generalizes the old `session.branchId !== user.branchId` re-validation:
// compares whichever of branchId/regionId/zoneId is set on the session
// against the current user's matching id.
function sessionMatchesUser(
  session: { branchId: string | null; regionId: string | null; zoneId: string | null },
  user: AuthTokenPayload
): boolean {
  if (session.branchId !== null) return session.branchId === user.branchId;
  if (session.regionId !== null) return session.regionId === user.regionId;
  if (session.zoneId !== null) return session.zoneId === user.zoneId;
  return false;
}
...
// suggestions, returned to the proposer for review.
export async function extractFromTranscript(
  user: AuthTokenPayload,
  transcript: string
): Promise<VoiceExtractionResult> {
  const scope = resolveVoiceScope(user);
  ...
  const authorizedLeads = await findLeadsInScope(scope);
...
// -> extraction -> lead ID + proposed stage -> proposer reviews ->
// confirms" pipeline, condensed into a single request for the case where
// the mobile client wants one round trip instead of orchestrating
...
// state... only CONFIRMED updates may modify the lead." The proposer
// must still confirm each proposal (individually or via the existing
// batch-confirm endpoint) before it actually changes a lead's stage. The
// response below reports proposal status accordingly (PENDING_REVIEW,
// not "updated") — an intentional, documented departure from a literal
...
export interface VoiceLeadUpdateResultItem {
  ...
  // flagged, not blocked, so the proposer sees it on the existing review
...
export async function processVoiceLeadUpdate(
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // A create failure here (e.g. a race where the lead's org
      // assignment changed) is reported as unresolved rather than
      // silently dropped — the API must never claim a clean result when
      // a step actually failed.
      unresolved.push({
...
// Step 2: the proposer has reviewed the candidates (resolved any
// ambiguity client-side, or accepted the extractor's own resolution) and
// is submitting the ones they actually want turned into real, PENDING
